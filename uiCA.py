...
class UopProperties:
    def __init__(
        ...
        divCycles=0,
        isLoadUop=False,
        isStoreAddressUop=False,
        memAddr=None,
        isStoreDataUop=False,
        isFirstUopOfInstr=False,
        isLastUopOfInstr=False,
        isRegMergeUop=False,
    ):
        self.instr = instr
    ...
    def __str__(self):
        return "UopProperties(ports: {}, in: {}, out: {}, lat: {})".format(
            self.possiblePorts, self.inputOperands, self.outputOperands, self.latencies
        )
...
class Uop:
    ...
    def __init__(self, prop, instrI):
        ...
        self.fusedUop: Optional[FusedUop] = (
            None  # fused-domain uop that contains this uop
        )
    ...
    def __str__(self):
        return "Uop(idx: {}, rnd: {}, p: {})".format(
            self.idx, self.instrI.rnd, self.actualPort
        )
...
class FusedUop:
    def __init__(self, uops: List[Uop]):
        ...
        for uop in uops:
            uop.fusedUop = self
        self.laminatedUop: Optional[LaminatedUop] = (
            None  # laminated-domain uop that contains this
        )
        self.issued = None  # cycle in which this uop was issued
        self.retired = None  # cycle in which this uop was retired
        self.retireIdx = (
            None  # how many other uops were already retired in the same cycle
        )
...
class LaminatedUop:
    ...
    def getUnfusedUops(self):
        return [
            uop for fusedUop in self.getFusedUops() for uop in fusedUop.getUnfusedUops()
        ]
...
class Renamer:
    def __init__(self, IDQ, reorderBuffer, uArchConfig: MicroArchConfig, initPolicy):
        ...
        self.lastRegMergeIssued = (
            None  # last uop for which register merge uops were issued
        )
    ...
    def cycle(self):
            ...
            if firstUnfusedUop.prop.isFirstUopOfInstr and regMergeProps:
                ...
                if self.lastRegMergeIssued != firstUnfusedUop:
                    for mergeProp in regMergeProps:
                        ...
                        firstUnfusedUop.instrI.regMergeUops.append(
                            LaminatedUop([mergeUop])
                        )
                    self.lastRegMergeIssued = firstUnfusedUop
                    break

            if (
                firstUnfusedUop.prop.isFirstUopOfInstr
                and firstUnfusedUop.prop.instr.isSerializingInstr
                and not self.reorderBuffer.isEmpty()
            ):
                break
        ...
        for fusedUop in renamerUops:
            for uop in fusedUop.getUnfusedUops():
                if (
                    uop.prop.instr.mayBeEliminated
                    and (not uop.prop.isRegMergeUop)
                    and (not isinstance(uop, StackSyncUop))
                ):
                    canonicalInpReg = getCanonicalReg(
                        uop.prop.instr.inputRegOperands[0].reg
                    )
                    ...
                    if canonicalInpReg in GPRegs:
                        if self.uArchConfig.moveEliminationGPRSlots == "unlimited":
                            ...
                            nGPRMoveElimPossible = (
                                self.uArchConfig.moveEliminationGPRSlots
                                - nGPRMoveElim
                                - sum(
                                    self.nGPRMoveElimInCycle.get(
                                        self.renamerActiveCycle - i, 0
                                    )
                                    for i in range(
                                        1,
                                        self.uArchConfig.moveEliminationPipelineLength,
                                    )
                                )
                                - self.multiUseGPRDictUseInCycle.get(
                                    self.renamerActiveCycle
                                    - self.uArchConfig.moveEliminationPipelineLength,
                                    0,
                                )
                            )
                        ...
                        if self.uArchConfig.moveEliminationSIMDSlots == "unlimited":
                            ...
                            nSIMDMoveElimPossible = (
                                self.uArchConfig.moveEliminationSIMDSlots
                                - nSIMDMoveElim
                                - sum(
                                    self.nSIMDMoveElimInCycle.get(
                                        self.renamerActiveCycle - i, 0
                                    )
                                    for i in range(
                                        1,
                                ...
                                - self.multiUseSIMDDictUseInCycle.get(
                                    self.renamerActiveCycle
                                    - self.uArchConfig.moveEliminationPipelineLength,
                                    0,
                                )
        ...
        if (nGPRMoveElim == 0) and (
            not self.uArchConfig.moveEliminationGPRAllAliasesMustBeOverwritten
        ):
        ...
        for fusedUop in renamerUops:
            for uop in fusedUop.getUnfusedUops():
                if uop.eliminated:
                    is32BitMove = (
                        getRegSize(uop.prop.instr.outputRegOperands[0].reg) == 32
                    )
                    canonicalInpReg = getCanonicalReg(
                        uop.prop.instr.inputRegOperands[0].reg
                    )
                    canonicalOutReg = getCanonicalReg(
                        uop.prop.instr.outputRegOperands[0].reg
                    )
                    ...
                    entry = self.renameDict.setdefault(
                        canonicalInpReg,
                        RenameDictEntry(RenamedOperand(ready=-1), False),
                    )
                    self.curInstrRndRenameDict[canonicalOutReg] = RenameDictEntry(
                        entry.renamedOp, is32BitMove
                    )
                    if entry.renamedOp not in curMultiUseDict:
                        curMultiUseDict[entry.renamedOp] = set()
                    curMultiUseDict[entry.renamedOp].update(
                        [canonicalInpReg, canonicalOutReg]
                    )
                    ...
                    if uop.prop.instr.uops or isinstance(uop, StackSyncUop):
                        ...
                        if uop.prop.isLoadUop:
                            ...
                            baseEntry = (
                                self.renameDict.get(getCanonicalReg(baseReg))
                                if baseReg
                                else None
                            )
                            baseInstr = (
                                baseEntry.renamedOp.uop.prop.instr
                                if (baseEntry and baseEntry.renamedOp.uop)
                                else None
                            )
                            indexReg = uop.prop.memAddr.get("index")
                            indexEntry = (
                                self.renameDict.get(getCanonicalReg(indexReg))
                                if indexReg
                                else None
                            )
                            indexInstr = (
                                indexEntry.renamedOp.uop.prop.instr
                                if (indexEntry and indexEntry.renamedOp.uop)
                                else None
                            )
                            uop.latReducedDueToFastPtrChasing = (
                                latReducedDueToFastPtrChasing(
                                    self.uArchConfig,
                                    uop.prop.memAddr,
                                    baseInstr,
                                    indexInstr,
                            ...
                            uop.storeBufferEntry = self.storeBufferEntryDict.get(
                                key, None
                            )
                        ...
                        for inpOp in uop.prop.inputOperands:
                            if isinstance(inpOp, PseudoOperand):
                                ...
                                renOp = self.renameDict.setdefault(
                                    key,
                                    RenameDictEntry(RenamedOperand(ready=-1), False),
                                ).renamedOp
                        ...
                        for outOp in uop.prop.outputOperands:
                            ...
                            if isinstance(outOp, PseudoOperand):
                                ...
                                self.curInstrRndRenameDict[key] = RenameDictEntry(
                                    renOp, False
                                )
                                if isinstance(outOp, RegOperand):
                                    self.absValGen.setAbstractValueForCurInstr(
                                        key, uop.prop.instr
                                    )
                    else:
                        # e.g., xor rax, rax
                        for op in uop.prop.instr.outputRegOperands:
                            self.curInstrRndRenameDict[getCanonicalReg(op.reg)] = (
                                RenameDictEntry(
                                    RenamedOperand(uop=uop, ready=-1), False
                                )
                            )

                if (
                    uop.prop.isLastUopOfInstr
                    or uop.prop.isRegMergeUop
                    or isinstance(uop, StackSyncUop)
                ):
                    for key in self.curInstrRndRenameDict:
                        if key in self.renameDict:
                            prevRenOp = self.renameDict[key].renamedOp
                            if (not uop.eliminated) or (
                                prevRenOp != self.curInstrRndRenameDict[key].renamedOp
                            ):
                                if (key in GPRegs) and (
                                    prevRenOp in self.multiUseGPRDict
                                ):
                                    self.multiUseGPRDict[prevRenOp].remove(key)
                                elif (
                                    (type(key) == str)
                                    and ("MM" in key)
                                    and (prevRenOp in self.multiUseSIMDDict)
                                ):
        ...
        if self.multiUseGPRDict:
            self.multiUseGPRDictUseInCycle[self.renamerActiveCycle] = len(
                self.multiUseGPRDict
            )
        ...
        if self.multiUseSIMDDict:
            self.multiUseSIMDDictUseInCycle[self.renamerActiveCycle] = len(
                self.multiUseSIMDDict
            )
    ...
    def getStoreBufferKey(self, memAddr):
        if memAddr is None:
            return None
        return (
            self.absValGen.getAbstractValueForReg(memAddr.get("base")),
            self.absValGen.getAbstractValueForReg(memAddr.get("index")),
            memAddr.get("scale"),
            memAddr.get("disp"),
        )
...
class FrontEnd:
    def __init__(
        ...
        if unroll or simpleFrontEnd:
            self.cacheBlockGenerator = CacheBlockGenerator(
                instructions, True, self.alignmentOffset
            )
        else:
            self.cacheBlocksForNextRoundGenerator = CacheBlocksForNextRoundGenerator(
                instructions, self.alignmentOffset
            )
            cacheBlocksForFirstRound = next(self.cacheBlocksForNextRoundGenerator)

            if self.uArchConfig.DSBBlockSize == 32:
                allBlocksCanBeCached = all(
                    canBeInDSB(block, uArchConfig.DSBBlockSize)
                    for cb in cacheBlocksForFirstRound
                    for block in split64ByteBlockTo32ByteBlocks(cb)
                    if block
                )
            else:
                allBlocksCanBeCached = all(
                    canBeInDSB(block, uArchConfig.DSBBlockSize)
                    for block in cacheBlocksForFirstRound
                )

            allInstrsCanBeUsedByLSD = all(
                instrI.instr.canBeUsedByLSD()
                for cb in cacheBlocksForFirstRound
                for instrI in cb
            )
            nUops = sum(
                len(instrI.uops) for cb in cacheBlocksForFirstRound for instrI in cb
            )
            if (
                allBlocksCanBeCached
                and self.uArchConfig.LSDEnabled
                and allInstrsCanBeUsedByLSD
                and (nUops <= self.uArchConfig.IDQWidth)
            ):
                self.uopSource = "LSD"
                self.LSDUnrollCount = self.uArchConfig.LSDUnrolling.get(nUops, 1)
                for cacheBlock in cacheBlocksForFirstRound + [
                    cb
                    for _ in range(0, self.LSDUnrollCount - 1)
                    for cb in next(self.cacheBlocksForNextRoundGenerator)
                ]:
    ...
    def cycle(self, clock):
        issueUops = []
        if (
            not self.reorderBuffer.isFull() and not self.scheduler.isFull()
        ):  # len(self.IDQ) >= uArchConfig.issueWidth and the first check seems to be wrong, but leads to better results
        ...
        if (
            len(self.instructionQueue) + self.uArchConfig.preDecodeWidth
            > self.uArchConfig.IQWidth
        ):
        ...
        if self.uopSource is None:
            ...
            while (
                len(self.DSB.DSBBlockQueue) < 2
                and len(self.preDecoder.B16BlockQueue) < 4
            ):
                if self.unroll:
            ...
            if self.MS.isBusy():
                ...
                if not self.unroll and newInstrIUops:
                    ...
                    if curInstrI.instr.isLastDecodedInstr() and (
                        curInstrI.instr.isBranchInstr
                        or curInstrI.instr.macroFusedWithNextInstr
                    ):
                        if self.alignmentOffset in self.addressesInDSB:
                            self.uopSource = "DSB"
                ...
                if newUops and newUops[-1].getUnfusedUops()[-1].prop.isLastUopOfInstr:
                    ...
                    if curInstrI.instr.isLastDecodedInstr():
                        ...
                        nextAddr = curInstrI.address + (
                            len(curInstrI.instr.opcode) // 2
                        )
    ...
    def findCacheableAddresses(self, cacheBlocksForFirstRound):
        for cacheBlock in cacheBlocksForFirstRound:
            if self.uArchConfig.DSBBlockSize == 32:
                splitCacheBlocks = [
                    block
                    for block in split64ByteBlockTo32ByteBlocks(cacheBlock)
                    if block
                ]
                if self.uArchConfig.both32ByteBlocksMustBeCacheable and any(
                    (not canBeInDSB(block, self.uArchConfig.DSBBlockSize))
            ...
            for block in blocks:
                ...
                if block[0].address in self.addressesInDSB:
                    ...
                    for B16Block in B16Blocks:
                        ...
                        if (
                            lastInstrI.instr.isBranchInstr
                            and (lastInstrI.address % 16)
                            + (len(lastInstrI.instr.opcode) // 2)
                            > 16
                        ):
    ...
    def addStackSyncUop(self, clock, uop):
        ...
        if self.RSPOffset and any(
            (getCanonicalReg(op.reg) == "RSP")
            for op in instr.inputRegOperands + instr.memAddrOperands
            if not op.isImplicitStackOperand
        ):
...
DSBEntry = namedtuple(
    "DSBEntry", ["slot", "instrI", "uop", "MSUops", "requiresExtraEntry"]
)
...
class DSB:
    ...
    def cycle(self):
        ...
        while remainingSlots > 0:
            if not DSBBlock:
                if (
                    (not secondDSBBlockLoaded)
                    and self.DSBBlockQueue
                    and (not self.DSBBlockQueue[0][-1].MSUops)
                ):
                    secondDSBBlockLoaded = True
                    DSBBlock = self.DSBBlockQueue[0]
                    prevInstrI = retList[-1][0]
                    if (
                        prevInstrI.address + len(prevInstrI.instr.opcode) / 2
                        != DSBBlock[0].instrI.address
                    ) and not prevInstrI.instr.isLastDecodedInstr():
            ...
            if not DSBBlock:
                ...
                if (
                    remainingSlots
                    and self.DSBBlockQueue
                    and (self.uArchConfig.DSBWidth == 6)
                ):
                    nextInstrAddr = self.DSBBlockQueue[0][0].instrI.address
                    nextInstrInSameMemoryBlock = (
                        nextInstrAddr // self.uArchConfig.DSBBlockSize
                        == entry.instrI.address // self.uArchConfig.DSBBlockSize
                    )
                    if (
                        (self.uArchConfig.DSBBlockSize == 32)
                        and nextInstrInSameMemoryBlock
                        and entry.instrI.instr.isLastDecodedInstr()
                        and (not delayInPrevCycle)
                    ):
                        remainingSlots = 0
                        ...
                        if newDSBBlockStarted:
                            ...
                            if (len(retList) == 1) or (
                                (len(retList) == 2) and (entry.slot >= 4)
                            ):
                                remainingSlots = 4
                            else:
...
class Decoder:
    def __init__(
        self, instructionQueue, MS: MicrocodeSequencer, uArchConfig: MicroArchConfig
    ):
    ...
    def cycle(self, clock):
        ...
        while self.instructionQueue:
            ...
            if instrI.instr.macroFusibleWith:
                if (not self.uArchConfig.macroFusibleInstrCanBeDecodedAsLastInstr) and (
                    nDecodedInstrs == self.uArchConfig.nDecoders - 1
                ):
                    break
                if (len(self.instructionQueue) <= 1) or (
                    self.instructionQueue[1].predecoded
                    + self.uArchConfig.predecodeDecodeDelay
                    > clock
                ):
                    break
            self.instructionQueue.popleft()
            ...
            if instrI.instr.complexDecoder:
                remainingDecoderSlots = min(
                    remainingDecoderSlots - 1, instrI.instr.nAvailableSimpleDecoders
                )
            else:
...
class PreDecoder:
    def __init__(self, instructionQueue, uArchConfig: MicroArchConfig):
        ...
        self.B16BlockQueue = (
            deque()
        )  # a deque of 16 Byte blocks (i.e., deques of InstrInstances)
        self.instructionQueue = instructionQueue
        self.curBlock = None
    ...
    def cycle(self, clock):
        if not self.stalled:
            if (
                (not self.preDecQueue)
                and (self.B16BlockQueue or self.partialInstrI)
                and len(self.instructionQueue) + self.uArchConfig.preDecodeWidth
                <= self.uArchConfig.IQWidth
            ):
                ...
                if not self.curBlock:
                    if (
                        len(self.B16BlockQueue)
                        < self.uArchConfig.preDecodeBlockSize // 16
                    ):
                        return
                    self.curBlock = deque()
                    ...
                    self.stalled = max(
                        0,
                        sum(3 for ii in self.curBlock if ii.instr.lcpStall)
                        - max(0, self.nonStalledPredecCyclesForCurBlock - 1),
                    )
                    self.nonStalledPredecCyclesForCurBlock = 0

                while (
                    self.curBlock
                    and len(self.preDecQueue) < self.uArchConfig.preDecodeWidth
                ):
                    if instrInstanceCrossesPredecBlockBoundary(
                        self.curBlock[0], self.uArchConfig.preDecodeBlockSize
                    ):
                        break
                ...
                if len(self.curBlock) == 1:
                    ...
                    if instrInstanceCrossesPredecBlockBoundary(
                        instrI, self.uArchConfig.preDecodeBlockSize
                    ):
                        offsetOfNominalOpcode = (
                            instrI.address % 16
                        ) + instrI.instr.posNominalOpcode
                        if (
                            len(self.preDecQueue) < self.uArchConfig.preDecodeWidth
                        ) or (offsetOfNominalOpcode >= 16):
    ...
    def isEmpty(self):
        return (
            (not self.B16BlockQueue)
            and (not self.preDecQueue)
            and (not self.partialInstrI)
        )
...
class ReorderBuffer:
    ...
    def retireUops(self, clock):
        ...
        for _ in range(0, self.uArchConfig.retireWidth):
            ...
            if all(
                (u.executed is not None and u.executed < clock) for u in unfusedUops
            ):
                self.uops.popleft()
...
class Scheduler:
    def __init__(self, uArchConfig: MicroArchConfig):
        ...
        self.blockedResources = (
            dict()
        )  # for how many remaining cycle a resource will be blocked
        self.blockedResources["div"] = 0
        self.dependentUops = (
            dict()
        )  # uops that have an operand that is written by a non-executed uop
    ...
    def dispatchUops(self, clock):
        ...
        if (
            ("4" in applicablePorts)
            and ("9" in applicablePorts)
            and self.readyQueue["4"]
            and self.readyQueue["9"]
        ):
        ...
        if (
            self.uArchConfig.slow256BitMemAcc
            and self.readyQueue["2"]
            and self.readyQueue["3"]
        ):
            uop2 = self.readyQueue["2"][0][1]
            uop3 = self.readyQueue["3"][0][1]
            if (
                uop2.prop.isLoadUop
                and uop3.prop.isLoadUop
                and (
                    ("M256" in uop2.instrI.instr.instrStr)
                    or ("M256" in uop3.instrI.instr.instrStr)
                )
            ):
        ...
        for port in applicablePorts:
            ...
            if (
                port == "0"
                and (not self.blockedResources["div"])
                and self.readyDivUops
                and (
                    (not self.readyQueue["0"])
                    or self.readyDivUops[0][0] < self.readyQueue["0"][0][0]
                )
            ):
            ...
            if (
                self.uArchConfig.slow256BitMemAcc
                and (port == "4")
                and ("M256" in uop.instrI.instr.instrStr)
            ):
    ...
    def processPendingFences(self, clock):
        for queue, uopsSinceLastFence in [
            (self.pendingLoadFenceUops, self.loadUopsSinceLastLoadFence),
    ...
    def checkUopReady(self, clock, uop):
        ...
        if uop.prop.instr.isLoadSerializing:
            if uop.prop.isFirstUopOfInstr and (
                self.pendingLoadFenceUops[0] != uop
                or any(
                    (uop2.executed is None) or (uop2.executed > clock)
                    for uop2 in self.loadUopsSinceLastLoadFence
                )
            ):
                return False
        elif uop.prop.instr.isStoreSerializing:
            if uop.prop.isFirstUopOfInstr and (
                self.pendingStoreFenceUops[0] != uop
                or any(
                    (uop2.executed is None) or (uop2.executed > clock)
                    for uop2 in self.storeUopsSinceLastStoreFence
                )
            ):
                return False
        else:
            if (
                uop.prop.isLoadUop
                and self.pendingLoadFenceUops
                and self.pendingLoadFenceUops[0].idx < uop.idx
            ):
                return False
            if (
                (uop.prop.isStoreDataUop or uop.prop.isStoreAddressUop)
                and self.pendingStoreFenceUops
                and self.pendingStoreFenceUops[0].idx < uop.idx
            ):
                return False

        if (
            uop.prop.isFirstUopOfInstr
            and self.blockedResources.get(uop.prop.instr.instrStr, 0) > 0
        ):
            return False
        ...
        for issueSlot, fusedUop in enumerate(newUops):
            for uop in fusedUop.getUnfusedUops():
                ...
                if len(uop.prop.possiblePorts) == 1:
                    ...
                    applicablePortUsages = [
                        (p, u)
                        for p, u in self.portUsageAtStartOfCycle.get(
                            clock - 1, self.portUsageAtStartOfCycle[clock]
                        ).items()
                        if p in uop.prop.possiblePorts
                    ]
                    sortedPortUsages = sorted(
                        applicablePortUsages, key=lambda x: (x[1], -int(x[0], 16))
                    )
                    minPortUsage = sortedPortUsages[0][1]
                    sortedPorts = [
                        p for p, u in sortedPortUsages if u < minPortUsage + 5
                    ]
                    ...
                    applicablePortUsages = [
                        (p, u)
                        for p, u in self.portUsageAtStartOfCycle[clock].items()
                        if p in uop.prop.possiblePorts
                    ]
                    minPort, minPortUsage = min(
                        applicablePortUsages, key=lambda x: (x[1], -int(x[0], 16))
                    )  # port with minimum usage so far
                    ...
                    if uop.prop.possiblePorts == ["2", "3"]:
                        ...
                        remApplicablePortUsages = [
                            (p, u) for p, u in applicablePortUsages if p != minPort
                        ]
                        min2Port, min2PortUsage = min(
                            remApplicablePortUsages,
                            key=lambda x: (x[1], -int(x[0], 16)),
                        )  # port with second smallest usage so far
                    ...
                    if uop.prop.possiblePorts == ["2", "3"]:
                        ...
                        (abs(u1 - u2) >= 3)
                        for _, u1 in applicablePortUsages
                        for _, u2 in applicablePortUsages
                    ):
                        port = minPort
    ...
    def checkDependingUopsExecuted(self, uop):
        for renInpOp in uop.renamedInputOperands:
            if (
                (renInpOp.getReadyCycle() is None)
                and renInpOp.uop
                and (renInpOp.uop.executed is None)
            ):
    ...
    def getReadyForDispatchCycle(self, clock, uop):
        ...
        if opReadyCycle < uop.fusedUop.issued + self.uArchConfig.issueDispatchDelay:
            ...
            (opReadyCycle == uop.fusedUop.issued + self.uArchConfig.issueDispatchDelay)
            or (
                opReadyCycle
                == uop.fusedUop.issued + self.uArchConfig.issueDispatchDelay + 1
            )
        ):  # ToDo: is second condition correct on HSW (ex: dec r10; add r11,0x8; test r10,r10)?
...
def adjustLatenciesAndAddMergeUops(
    instructions: List[Instr], uArchConfig: MicroArchConfig
):
    if uArchConfig.high8RenamedSeparately:
        ...
        def processInstrRegOutputs(instr):
            for op in (
                instr.inputRegOperands + instr.memAddrOperands + instr.outputRegOperands
            ):
                canonicalReg = getCanonicalReg(op.reg)
                if (canonicalReg in ["RAX", "RBX", "RCX", "RDX"]) and (
                    getRegSize(op.reg) > 8
                ):
        ...
        for instr in instructions:
            for inOp, outOp in instr.latencies:
                if (
                    isinstance(inOp, RegOperand)
                    and (inOp.reg in High8Regs)
                    and high8RegClean[getCanonicalReg(inOp.reg)]
                ):
            ...
            for inOp in instr.inputRegOperands + instr.memAddrOperands:
                ...
                if (
                    (canonicalInReg in ["RAX", "RBX", "RCX", "RDX"])
                    and (getRegSize(inOp.reg) > 8)
                    and (not high8RegClean[canonicalInReg])
                ):
                    canonicalInOp = RegOperand(canonicalInReg)
                    canonicalOutOp = RegOperand(canonicalInReg)
                    regMergeUopProp = UopProperties(
                        instr,
                        ["1", "5"],
                        [canonicalInOp],
                        [canonicalOutOp],
    ...
    for instr in instructions:
        ...
        for i, pc in enumerate(loadPcs):
            ...
            if instr.inputMemOperands:
                memAddr = instr.inputMemOperands[
                    min(i, len(instr.inputMemOperands) - 1)
                ].memAddr
            uopLatencies = {
                outOp: 5 for outOp in outputOperands
            }  # ToDo: actual latencies
            loadUopProps.append(
                UopProperties(
                    instr,
                    pc,
        ...
        for i, (stAPc, stDPc) in enumerate(zip(storeAddressPcs, storeDataPcs)):
            ...
            storeUopProps.append(
                UopProperties(
                    instr, stDPc, staDInputOperands, [], {}, isStoreDataUop=True
                )
            )

        if nonMemPcs:
            if (
                (not instr.memAddrOperands)
                and (len(nonMemPcs) == 3)
                and instr.inputRegOperands
                and instr.outputRegOperands
                and instr.inputFlagOperands
                and instr.outputFlagOperands
                and all(
                    instr.latencies.get((i, o)) == 1
                    for i in instr.inputRegOperands
                    for o in instr.outputRegOperands
                )
                and all(
                    instr.latencies.get((i, o)) == 2
                    for i in instr.inputRegOperands
                    for o in instr.outputFlagOperands
                )
                and all(
                    instr.latencies.get((i, o)) == 0
                    for i in instr.inputFlagOperands
                    for o in instr.outputRegOperands
                )
                and all(
                    instr.latencies.get((i, o)) == 2
                ...
                nonMemUopProps.append(
                    UopProperties(
                        instr,
                        nonMemPcs[0],
                        instr.inputRegOperands,
                        rOutputOperands,
                        rLat,
                    )
                )
                ...
                nonMemInputOperands = (
                    instr.inputRegOperands
                    + instr.inputFlagOperands
                    + (instr.memAddrOperands if instr.agenOperands else [])
                    + loadPseudoOps
                )
                nonMemOutputOperands = (
                    instr.outputRegOperands + instr.outputFlagOperands + storePseudoOps
                )
                ...
                for inOp in (
                    ...
                    for outOp in instr.outputRegOperands + instr.outputFlagOperands:
                        adjustedLatencies[(inOp, outOp)] = instr.latencies.get(
                            (inOp, outOp), 1
                        )
                    for storePseudoOp in storePseudoOps:
                        adjustedLatencies[(inOp, storePseudoOp)] = max(
                            [
                                max(1, instr.latencies.get((inOp, outMemOp), 1) - 4)
                                for outMemOp in instr.outputMemOperands
                            ]
                            or [1]
                        )  # ToDo
                for inMemAddrOp in (
                    instr.memAddrOperands if (not instr.agenOperands) else []
                ):
                    for loadPseudoOp in loadPseudoOps:
                        for outOp in instr.outputRegOperands + instr.outputFlagOperands:
                            adjustedLatencies[(loadPseudoOp, outOp)] = max(
                                1, instr.latencies.get((inMemAddrOp, outOp), 1) - 5
                            )  # ToDo
                for inMemOp in instr.inputMemOperands:
                    for loadPseudoOp in loadPseudoOps:
                        for storePseudoOp in storePseudoOps:
                            adjustedLatencies[(loadPseudoOp, storePseudoOp)] = max(
                                [
                                    max(
                                        1,
                                        instr.latencies.get((inMemOp, outMemOp), 1) - 5,
                                    )
                                    for outMemOp in instr.outputMemOperands
                                ]
                                or [1]
                ...
                for inOp in nonMemInputOperands:
                    latValues = set(
                        adjustedLatencies.get((inOp, outOp), 1)
                ...
                minLatLevel = (
                    remainingLatClassLevels.popleft() if remainingLatClassLevels else 1
                )
                minLatClass = latClasses.get(minLatLevel, [])
                for outOp in nonMemOutputOperands:
                    if minLatClass:
                        baseUopLatencies[outOp] = max(
                            adjustedLatencies.get((inOp, outOp), 1)
                            for inOp in minLatClass
                        )
                    else:
                        baseUopLatencies[outOp] = 1

                baseUopProp = UopProperties(
                    instr,
                    nonMemPcs[0],
                    minLatClass,
                    nonMemOutputOperands,
                ...
                for i, pc in enumerate(nonMemPcs[1:]):
                    if remainingLatClassLevels:
                        ...
                        nonMemUopProps.appendleft(
                            UopProperties(instr, pc, latClass, [pseudoOp], latDict)
                        )
                    else:
                        nonMemUopProps.append(
                            UopProperties(instr, pc, nonMemInputOperands, [], {})
                        )
...
class InstrInstance:
    def __init__(self, instr, address, rnd):
        ...
        self.removedFromIQ = (
            None  # cycle in which the instruction instance was removed from the IQ
        )
    ...
    def __generateUops(self):
        ...
        unfusedDomainUops = deque(
            [Uop(prop, self) for prop in self.instr.UopPropertiesList]
        )
        ...
        for i in range(0, self.instr.retireSlots - 1):
            ...
            if (
                uop.prop.possiblePorts
                and any(p in ["2", "3", "7"] for p in uop.prop.possiblePorts)
        ...
        laminatedDomainUops = []
        nLaminatedDomainUops = min(
            self.instr.uopsMITE + self.instr.uopsMS, len(fusedDomainUops)
        )
        for i in range(0, nLaminatedDomainUops - 1):
            fusedUop = fusedDomainUops.popleft()
            if (
                (len(fusedUop.getUnfusedUops()) == 1)
                and fusedUop.getUnfusedUops()[0].prop.possiblePorts
                and any(
                    p in ["2", "3", "7"]
                    for p in fusedUop.getUnfusedUops()[0].prop.possiblePorts
                )
                and len(fusedDomainUops) >= nLaminatedDomainUops - i
            ):
                laminatedDomainUops.append(
                    LaminatedUop([fusedUop, fusedDomainUops.popleft()])
                )
            else:
                laminatedDomainUops.append(LaminatedUop([fusedUop]))
        laminatedDomainUops.append(
            LaminatedUop(list(fusedDomainUops))
        )  # add remaining uops
...
def split64ByteBlockTo16ByteBlocks(cacheBlock):
    return [
        [ii for ii in cacheBlock if b * 16 <= ii.address % 64 < (b + 1) * 16]
        for b in range(0, 4)
    ]
...
def split32ByteBlockTo16ByteBlocks(B32Block):
    return [
        [ii for ii in B32Block if b * 16 <= ii.address % 32 < (b + 1) * 16]
        for b in range(0, 2)
    ]
...
def split64ByteBlockTo32ByteBlocks(cacheBlock):
    return [
        [ii for ii in cacheBlock if b * 32 <= ii.address % 64 < (b + 1) * 32]
        for b in range(0, 2)
    ]
    ...
    for instrI in cacheBlock:
        ...
        if instr.immediate is not None:
            if not (-(2**31) <= instr.immediate < 2**31):
                ...
                not (-(2**15) <= instr.immediate < 2**15)
                and len(instr.memAddrOperands) > 0
            ):
                requiresExtraEntry = True

        if instr.uopsMS or (
            nRequiredEntries + int(requiresExtraEntry) > remainingEntriesInCurBlock
        ):
            curBlock = deque()
        ...
        if instr.uopsMITE:
            ...
            curBlock.append(
                DSBEntry(len(curBlock), instrI, None, list(instrI.uops), False)
            )
        else:
...
def canBeInDSB(block, DSBBlockSize):
    ...
    for B32Block in B32Blocks:
        ...
        if (
            B16_1
            and B16_2
            and ((B16_1[-1].address % 16) + B16_1[-1].instr.posNominalOpcode >= 16)
        ):
            B16_2.insert(0, B16_1.pop())
        if (
            B16_1
            and B16_1[-1].instr.lcpStall
            and B16_1[-1].instr.macroFusibleWith
            and (
                len(
                    [
                        instrI
                        for instrI in B16_2
                        if instrI.instr.lcpStall and instrI.instr.macroFusibleWith
                    ]
                )
                >= 2
            )
...
TableLineData = NamedTuple(
    "TableLineData",
    [
        ("string", str),
        ("instr", Optional[Instr]),
        ("url", Optional[str]),
        ("uopsForRnd", List[List[LaminatedUop]]),
    ],
)


def getUopsTableColumns(
    tableLineData: List[TableLineData], uArchConfig: MicroArchConfig
):
    ...
    if any(
        ...
        for lamUop in tld.uopsForRnd[0]
        for uop in lamUop.getUnfusedUops()
    ):
...
def printUopsTable(tableLineData, uArchConfig: MicroArchConfig, addHyperlink=True):
    ...
    if "Notes" in columns:
        if "J" in columns["Notes"]:
            ...
            print(
                "J - Block not in DSB due to "
                + getTerminalHyperlink(jccURL, "JCC erratum")
            )
        if "M" in columns["Notes"]:
            print("M - Macro-fused with previous instruction")
        if "X" in columns["Notes"]:
            print("X - Instruction not supported")
        print("")

    columnWidthList = [
        2 + max(len(k), max(len(formatTableValue(l)) for l in lines))
        for k, lines in columns.items()
    ]
    ...
    print(getTableBorderLine("\u251c", "\u253c", "\u2524"))
    sumLine = getTableLine(
        [formatTableValue(sum(v) if k != "Notes" else "") for k, v in columns.items()]
    )
    sumLine += " Total"
    print(sumLine)
    print(getTableBorderLine("\u2514", "\u2534", "\u2518"))
...
def getBottlenecks(
    ...
    allLamUops = [
        lamUop
        for iiList in instrInstancesForInstr.values()
        for ii in iiList
        for lamUop in ii.uops + ii.regMergeUops + ii.stackSyncUops
    ]
    ...
    miteInstrs = [
        ...
        if instrInstancesForInstr[i] and (instrInstancesForInstr[i][0].source == "MITE")
    ]
    dsbInstrs = [
        i
        for i in instructions
        if instrInstancesForInstr[i] and (instrInstancesForInstr[i][0].source == "DSB")
    ]
    lsdInstrs = [
        i
        for i in instructions
        if instrInstancesForInstr[i] and (instrInstancesForInstr[i][0].source == "LSD")
    ]

    if miteInstrs:
        if (not dsbInstrs) and (not lsdInstrs):
            limits["predecoder"] = round(
                computePredecLimit(disas, loop, alignmentOffset), 2
            )
            if limits["predecoder"]:
                output.append("  - Predecoder: {:.2f}".format(limits["predecoder"]))
                if limits["predecoder"] >= 0.98 * TP:
...
def printBottlenecks(
    ...
    bottlenecks, output, _ = getBottlenecks(
        TP,
        instructions,
        instrInstancesForInstr,
        disas,
    ...
    print(
        "Bottleneck"
        + ("s" if len(bottlenecks) > 1 else "")
        + ": "
        + (", ".join(bottlenecks) if bottlenecks else "unknown")
    )
    if output:
        print("")
        print(
...
def generateHTMLTraceTable(
    filename, instructions, instrInstances, lastRelevantRound, maxCycle
):
    import json
    ...
    for instrI in instrInstances:
        ...
        if instrI.regMergeUops:
            subInstrs += [
                (
                    ...
                    [
                        uop
                        for lamUop in instrI.regMergeUops
                        for uop in lamUop.getUnfusedUops()
                    ],
                )
            ]
        if instrI.stackSyncUops:
            subInstrs += [
                (
                    ...
                    [
                        uop
                        for lamUop in instrI.stackSyncUops
                        for uop in lamUop.getUnfusedUops()
                    ],
                )
            ]
        if instrI.rnd == 0 and (not isinstance(instrI.instr, UnknownInstr)):
            string = '<a href="{}" target="_blank">{}</a>'.format(
                getURL(instrI.instr.instrStr), instrI.instr.asm
            )
        else:
        ...
        subInstrs += [
            (
                string,
                False,
                [uop for lamUop in instrI.uops for uop in lamUop.getUnfusedUops()],
            )
        ]
        ...
        for string, isPseudoInstr, uops in subInstrs:
            ...
            if not isPseudoInstr:
                preDec = (
                    instrI.predecoded
                    if not instrI.instr.macroFusedWithPrevInstr
                    else prevInstrI.predecoded
                )
            ...
            if not uops:
                ...
                for uopI, uop in enumerate(uops):
                    ...
                    uopData["possiblePorts"] = (
                        ("{" + ",".join(uop.prop.possiblePorts) + "}")
                        if uop.prop.possiblePorts
                        else "-"
                    )
                    ...
                    for evCycle, ev in [
                        (preDec, "P"),
                        (uop.fusedUop.laminatedUop.addedToIDQ, "Q"),
                        (uop.fusedUop.issued, "I"),
                        (uop.readyForDispatch, "r"),
                        (uop.dispatched, "D"),
                        (uop.executed, "E"),
                        (uop.fusedUop.retired, "R"),
                        ...
                        if (
                            (evCycle is not None)
                            and (evCycle >= 0)
                            and (evCycle <= maxCycle)
                        ):
                            uopData["events"][evCycle] = ev
        prevInstrI = instrI

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "traceTemplate.html"),
        "r",
    ) as t:
        html = t.read()
        html = html.replace(
            "var tableData = {}", "var tableData = " + json.dumps(tableDataForRnd)
        )
...
def generateHTMLGraph(
    filename,
    instructions,
    instrInstances: List[InstrInstance],
    uArchConfig: MicroArchConfig,
    maxCycle,
):
    ...
    for evtName, evtAttrName in [
        ("&mu;ops issued", "issued"),
        ("&mu;ops retired", "retired"),
    ]:
        eventsDict[evtName] = [0 for _ in range(0, maxCycle + 1)]
        for instrI in instrInstances:
            for lamUop in instrI.uops:
    ...
    for evtName, evtAttrName in [
        ("&mu;ops dispatched", "dispatched"),
        ("&mu;ops executed", "executed"),
    ]:
        eventsDict[evtName] = [0 for _ in range(0, maxCycle + 1)]
        for instrI in instrInstances:
            for lamUop in instrI.uops:
    ...
    for evtName, events in eventsDict.items():
        ...
        fig.add_trace(
            go.Scatter(
                y=cumulativeEvents, mode="lines+markers", line_shape="hv", name=evtName
            )
        )
    ...
    writeHtmlFile(
        filename, "Graph", head, body, includeDOCTYPE=False
...
def generateJSONOutput(
    ...
    for instrI in frontEnd.allGeneratedInstrInstances:
        ...
        if (instrI.predecoded is not None) and (instrI.predecoded <= maxCycle):
            cycles[instrI.predecoded].setdefault("addedToIQ", []).append(
                {"rnd": rnd, "instr": instrID}
            )
        if (instrI.removedFromIQ is not None) and (instrI.removedFromIQ <= maxCycle):
            cycles[instrI.removedFromIQ].setdefault("removedFromIQ", []).append(
                {"rnd": rnd, "instr": instrID}
            )

        for lamUopI, lamUop in enumerate(
            instrI.regMergeUops + instrI.stackSyncUops + instrI.uops
        ):
            baseUopDict = {
                "rnd": rnd,
                "instrID": instrID,
            ...
            if (lamUop.addedToIDQ is not None) and (lamUop.addedToIDQ <= maxCycle):
                ...
                cycles[lamUop.addedToIDQ].setdefault("addedToIDQ", []).append(
                    lamUopDict
                )
            ...
            for fUopI, fUop in enumerate(lamUop.getFusedUops()):
                ...
                for uopI, uop in enumerate(fUop.getUnfusedUops()):
                    ...
                    if (
                        (fUop.issued is not None)
                        and (fUop.issued <= maxCycle)
                        and (fUop.issued != uop.executed)
                    ):
                    ...
                    if (uop.readyForDispatch is not None) and (
                        uop.readyForDispatch <= maxCycle
                    ):
                        cycles[uop.readyForDispatch].setdefault(
                            "readyForDispatch", []
                        ).append(unfusedUopDict)
                    if (uop.dispatched is not None) and (uop.dispatched <= maxCycle):
                        cycles[uop.dispatched].setdefault("dispatched", {})[
                            "Port" + uop.actualPort
                        ] = unfusedUopDict
                    if (uop.executed is not None) and (uop.executed <= maxCycle):
                        cycles[uop.executed].setdefault("executed", []).append(
                            unfusedUopDict
                        )
...
def getURL(instrStr):
    return (
        "https://www.uops.info/html-instr/"
        + canonicalizeInstrString(instrStr)
        + ".html"
    )


# Returns the throughput
def generateEventTrace(filename, instructions, frontEnd, uArchConfig, maxCycle):
    # Stable textual event trace for parity debugging.
    # Format (one event per line):
    #   C<cycle> <EV> instr=<instrID> rnd=<rnd> lam=<lamUopID> fused=<fUopID> uop=<uopID> [port=<port>] [source=<source>]
    # Events:
    #   P  predecoded       (per instrI; no lam/fused/uop)
    #   X  removedFromIQ    (per instrI)
    #   Q  addedToIDQ       (per lamUop; source tag)
    #   I  issued           (per fUop; aka addedToRB/removedFromIDQ)
    ...
    for instrI in frontEnd.allGeneratedInstrInstances:
        ...
        if (instrI.predecoded is not None) and (instrI.predecoded <= maxCycle):
            events.append(
                (instrI.predecoded, 0, "P", instrID, rnd, -1, -1, -1, None, None)
            )
        if (instrI.removedFromIQ is not None) and (instrI.removedFromIQ <= maxCycle):
            events.append(
                (instrI.removedFromIQ, 1, "X", instrID, rnd, -1, -1, -1, None, None)
            )
        lamUopList = instrI.regMergeUops + instrI.stackSyncUops + instrI.uops
        for lamI, lamUop in enumerate(lamUopList):
            if (lamUop.addedToIDQ is not None) and (lamUop.addedToIDQ <= maxCycle):
                events.append(
                    (
                        lamUop.addedToIDQ,
            ...
            for fI, fUop in enumerate(lamUop.getFusedUops()):
                if (fUop.issued is not None) and (fUop.issued <= maxCycle):
                    events.append(
                        (fUop.issued, 3, "I", instrID, rnd, lamI, fI, -1, None, None)
                    )
                if (fUop.retired is not None) and (fUop.retired <= maxCycle):
                    events.append(
                        (fUop.retired, 7, "R", instrID, rnd, lamI, fI, -1, None, None)
                    )
                for uI, uop in enumerate(fUop.getUnfusedUops()):
                    if (uop.readyForDispatch is not None) and (
                        uop.readyForDispatch <= maxCycle
                    ):
                        events.append(
                            (
                                uop.readyForDispatch,
                                4,
                                "r",
                                instrID,
                                rnd,
                    ...
                    if (uop.dispatched is not None) and (uop.dispatched <= maxCycle):
                        port = ("Port" + uop.actualPort) if uop.actualPort else "-"
                        events.append(
                            (
                                uop.dispatched,
                                5,
    ...
    events.sort(key=lambda e: (e[0], e[1], e[3], e[4], e[5], e[6], e[7]))

    with open(filename, "w") as f:
        for cycle, _order, ev, instrID, rnd, lam, fused, uop, port, source in events:
            line = "C{} {} instr={} rnd={} lam={} fused={} uop={}".format(
                cycle, ev, instrID, rnd, lam, fused, uop
            )
            if port is not None:
                line += " port=" + port
...
def runSimulation(
    ...
    instructions = getInstructions(
        disas,
        uArchConfig,
        importlib.import_module("instrData." + uArchConfig.name + "_data"),
        alignmentOffset,
        noMicroFusion,
        noMacroFusion,
    )
    ...
    lastApplicableInstr = next(
        instr for instr in instructions if instr.isLastDecodedInstr()
    )  # ignore macro-fused instr.
    lastRelevantRound = max(
        0, len(uopsForRound) - 2
    )  # last round may be incomplete, thus -2
    ...
    if lastRelevantRound - firstRelevantRound > 10:
        for rnd in range(lastRelevantRound, lastRelevantRound - 5, -1):
            if (
                uopsForRound[firstRelevantRound][lastApplicableInstr][-1].retireIdx
                == uopsForRound[rnd][lastApplicableInstr][-1].retireIdx
            ):
    ...
    if firstRelevantRound == lastRelevantRound:
        ...
        TP = round(
            (
                uopsForRelRound[-1][lastApplicableInstr][-1].retired
                - uopsForRelRound[0][lastApplicableInstr][-1].retired
            )
            / (len(uopsForRelRound) - 1),
            2,
        )
    ...
    if printDetails or (depGraphFile is not None) or (jsonFile is not None):
        nodesForInstr, edgesForNode = generateLatencyGraph(
            instructions, uArchConfig, initPolicy
        )
        maxCycleRatio, edgesOnMaxCycle, comp = computeMaximumLatencyForGraph(
            instructions, nodesForInstr, edgesForNode
        )
    ...
    if printDetails:
        ...
        for instr in instructions:
            ...
            if any(instrI.regMergeUops for instrI in instrInstances):
                ...
                tableLineData.append(
                    TableLineData("<Register Merge Uop>", None, None, uops)
                )
            if any(instrI.stackSyncUops for instrI in instrInstances):
                uops = [instrI.stackSyncUops for instrI in instrInstances]
                tableLineData.append(
                    TableLineData("<Stack Sync Uop>", None, None, uops)
                )
        ...
        printBottlenecks(
            TP,
            instructions,
            relevantInstrInstancesForInstr,
            disas,
            alignmentOffset,
            (not frontEnd.unroll),
            maxCycleRatio,
    ...
    if traceFile is not None:
        ...
        generateHTMLTraceTable(
            traceFile,
            instructions,
            frontEnd.allGeneratedInstrInstances,
            lastRelevantRound,
            clock - 1,
        )
    ...
    if depGraphFile is not None:
        generateGraphvizOutputForLatencyGraph(
            instructions,
            nodesForInstr,
            edgesForNode,
            edgesOnMaxCycle,
    ...
    if eventTraceFile is not None:
        generateEventTrace(
            eventTraceFile, instructions, frontEnd, uArchConfig, clock - 1
        )

    if jsonFile is not None:
        bottlenecks, _, limits = getBottlenecks(
            TP,
            instructions,
            relevantInstrInstancesForInstr,
            disas,
...
def main():
    allMicroArchs = sorted(m for m in MicroArchConfigs if "_" not in m)
    ...
    parser.add_argument(
        "-arch",
        help="Microarchitecture; Possible values: all, " + ", ".join(allMicroArchs),
        default="all",
    )
    ...
    parser.add_argument(
        "-TPonly", help="Output only the TP prediction", nargs="?", const="graph.html"
    )
    parser.add_argument(
        "-simpleFrontEnd",
        help="Simulate a simple front end that is only limited by the issue width",
        action="store_true",
    )
    parser.add_argument(
        "-noMicroFusion",
        help="Variant that does not support micro-fusion",
        action="store_true",
    )
    parser.add_argument(
        "-noMacroFusion",
        help="Variant that does not support macro-fusion",
        action="store_true",
    )
    parser.add_argument(
        "-alignmentOffset",
        help='Alignment offset (relative to a 64-Byte cache line), or "all"; default: 0',
        default="0",
    )
    parser.add_argument(
        "-minIterations",
        help="Simulate at least this many iterations; default: 10",
        type=int,
        default=10,
    )
    parser.add_argument(
        "-minCycles",
        help="Simulate at least this many cycles; default: 500",
        type=int,
        default=500,
    )
    ...
    parser.add_argument(
        "-eventTrace",
        help="Plain-text event trace for parity debugging",
        nargs="?",
        const="events.trace",
    )
    parser.add_argument(
        "-depGraph",
        help="Output the dependency graph; the format is determined by the filename extension",
        nargs="?",
        const="dep.svg",
    )
    parser.add_argument(
        "-initPolicy",
        help="Initial register state; "
    ...
    if args.arch not in list(MicroArchConfigs) + ["all"]:
        print("Unsupported microarchitecture")
        exit(1)
    if args.initPolicy not in ["diff", "same", "stack"]:
    ...
    if args.arch == "all":
        ...
        disasList = [
            xed.disasFile(
                args.filename,
                chip=MicroArchConfigs[uArch].XEDName,
                raw=args.raw,
                useIACAMarkers=args.iacaMarkers,
            )
            for uArch in allMicroArchs
        ]
        ...
        with futures.ProcessPoolExecutor() as executor:
            TPList = list(
                executor.map(
                    ...
                    disasList,
                    uArchConfigsList,
                    repeat(int(args.alignmentOffset)),
                    repeat(args.initPolicy),
                    repeat(args.noMicroFusion),
                    repeat(args.noMacroFusion),
                    repeat(args.simpleFrontEnd),
                    repeat(args.minIterations),
                    repeat(args.minCycles),
                )
            )
        TPDict = {}
        ...
        if len(TPDict.keys()) == 1:
            print(
                "Throughput (in cycles per iteration): {:.2f}".format(
                    next(iter(TPDict))
                )
            )
        else:
            print(
                "Throughput (in cycles per iteration): {:.2f} - {:.2f}\n".format(
                    min(TPDict), max(TPDict)
                )
            )
    ...
    if args.alignmentOffset == "all":
        ...
        if len(TPDict.keys()) == 1:
            ...
            for TP, alList in sortedTP[:-1]:
                print(
                    "    - {:.2f} for alignment offsets in {{{}}}".format(
                        TP, ", ".join(alList)
                    )
                )
